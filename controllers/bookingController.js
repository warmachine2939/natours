...
exports.createBookingCheckout = catchAsync(
  async (req, res, next) => {
    ...
    if (!tour || !user || !price || !startDateId) {
      return next();
    }

    const tourDoc = await Tour.findById(tour);

    const startDate = tourDoc.startDates.find(
      (startDates) =>
        startDates._id.toString() === startDateId.toString()
    );
    startDate.participants += 1;
    if (
      startDate.participants > startDate.maxParticipants
    ) {
      return next(
        new AppError(
          'Maximum participants reached for this start date',
          400
        )
      );
    }
...
exports.checkIfBooked = catchAsync(
  async (req, res, next) => {
    ...
    if (booking.length === 0)
      return next(
        new AppError(
          'You must buy this tour to review it',
          401
        )
      );
    next();
