...
if (reviewDataForm) reviewDataForm.addEventListener("submit", (e)=>{
    e.preventDefault();
    const review = document.getElementById("review").value;
    const rating = document.getElementById("rating").value;
    const { user, tour } = JSON.parse(reviewDataForm.dataset.ids);
    (0, _reviews.leaveReview)(review, rating, tour, user);
    document.getElementById("review").textContent = "";
    document.getElementById("rating").textContent = "";
});
if (reviews) reviews.addEventListener("click", (e)=>{
    if (e.target.tagName === "BUTTON") {
        ...
        if (button.textContent === "Delete") {
            ...
            (0, _reviews.deleteReview)(reviewId);
            setTimeout(()=>{
            ...
            reviewsCard.removeChild(inputReview);
            reviewsCard.removeChild(inputRating);
            (0, _reviews.editReview)(+inputRating.value, reviewText.textContent, button.dataset.reviewId);
...
var _alerts = require("./alerts");
/*eslint-disable*/ const stripe = Stripe("pk_test_51NSSWuSATxeCRZhs12oftZCZ0Vp3oBnE9URxKECGqTVCrwAtENHrEzM2x4fCuRlwz7dxECRRmCZYnGKA1Ydy82Ev00n8cEr4jt");
const bookTour = async (tourId, startDateId)=>{
    try {
...
const leaveReview = async (review, rating, tour, user)=>{
    try {
        ...
        if (res.data.status === "success") {
            (0, _alerts.showAlert)("success", "Review was succesfully uploaded!");
            window.setTimeout(()=>{
                location.reload(true);
            }, 1000);
        }
    } catch (err) {
        (0, _alerts.showAlert)("error", "You can leave only one review.");
    }
};
const editReview = async (rating, review, reviewId)=>{
    try {
        ...
        if (res.data.status === "success") {
            (0, _alerts.showAlert)("success", "Review was succesfully edited!");
            window.setTimeout(()=>{
                location.reload(true);
            }, 1000);
        }
    } catch (err) {
        (0, _alerts.showAlert)("error", "Error with editing review!");
    }
};
const deleteReview = async (reviewId)=>{
    try {
        ...
        if (res.status === 204) {
            (0, _alerts.showAlert)("success", "Review was succesfully deleted!");
            window.setTimeout(()=>{
                location.reload(true);
            }, 600);
        }
    } catch (err) {
        (0, _alerts.showAlert)("error", "Error with deleting review!");
    }
};
